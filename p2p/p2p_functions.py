...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(self, tokenizer, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
                 local_blend: Optional[LocalBlend] = None):
        super(AttentionRefine, self).__init__(tokenizer, prompts, num_steps, cross_replace_steps, self_replace_steps, local_blend)
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(self, tokeinzer, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float, equalizer,
                 local_blend: Optional[LocalBlend] = None, controller: Optional[AttentionControlEdit] = None):
        super(AttentionReweight, self).__init__(tokeinzer, prompts, num_steps, cross_replace_steps, self_replace_steps,
                                                local_blend)
...
def make_controller(tokenizer, prompts: List[str], is_replace_controller: bool, cross_replace_steps: Dict[str, float],
                    self_replace_steps: float, blend_words=None, equilizer_params=None,
                    num_ddim_steps=50) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(tokenizer,prompts, blend_words)
    if is_replace_controller:
        controller = AttentionReplace(tokenizer, prompts, num_ddim_steps, cross_replace_steps=cross_replace_steps,
                                      self_replace_steps=self_replace_steps, local_blend=lb)
    else:
        controller = AttentionRefine(tokenizer, prompts, num_ddim_steps, cross_replace_steps=cross_replace_steps,
                                     self_replace_steps=self_replace_steps, local_blend=lb)
    if equilizer_params is not None:
        eq = get_equalizer(tokenizer, prompts[1], equilizer_params["words"], equilizer_params["values"])
        controller = AttentionReweight(tokenizer, prompts, num_ddim_steps, cross_replace_steps=cross_replace_steps,
